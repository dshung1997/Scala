...
class Routes extends Directives {
  ...
  val getProfile = pathPrefix("profile") {
    path("get" / Segment) {
      case seg: String =>
        get {
          val exe = UserInfoTableDef.get(seg)
          onSuccess(exe) {
            case null => complete(StatusCodes.BadRequest, "Get profile failed with no Email")
            case userInfo: UserInfo => complete(StatusCodes.OK, userInfo)
          }
        }
    }
  }
  ...
  val routes: Route = getProfile ~ check
}
